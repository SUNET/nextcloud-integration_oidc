...
require('../assets/NcAvatar-CcJlFpcI.css');
"use strict";
...
const NcUserStatusIcon = require("./NcUserStatusIcon-CFLiHFkC.cjs");
const usernameToColor = require("./usernameToColor-cVFS7tET.cjs");
  ...
  computed: {
    avatarAriaLabel() {
      ...
      if (this.canDisplayUserStatus || this.showUserStatusIconOnAvatar) {
        return _l10n.t("Avatar of {displayName}, {status}", { displayName: this.displayName ?? this.user, status: NcUserStatusIcon.getUserStatusText(this.userStatus.status) });
      }
      return _l10n.t("Avatar of {displayName}", { displayName: this.displayName ?? this.user });
    },
    ...
    hasMenu() {
      ...
      return !(this.user === auth.getCurrentUser()?.uid || this.userDoesNotExist || this.url);
    },
    /**
    ...
    menu() {
      ...
      for (const action of Functions_contactsMenu.getEnabledContactsMenuActions(this.contactsMenuData)) {
        try {
          actions.push({
            ncActionComponent: NcActionButton.NcActionButton,
          ...
          logger.logger.error(`Failed to render ContactsMenu action ${action.id}`, {
            error,
            action
          });
        }
  ...
  methods: {
    ...
    async fetchContactsMenu() {
      ...
      try {
        ...
        const { data } = await axios__default.default.post(router.generateUrl("contactsmenu/findOne"), `shareType=0&shareWith=${user}`);
    ...
    avatarUrlGenerator(user, size) {
      ...
      if (user === auth.getCurrentUser()?.uid && typeof oc_userconfig !== "undefined") {
...
var _sfc_render = function render() {
  ...
  }), _vm.hasMenu && _vm.menu.length === 0 ? _c("NcButton", { staticClass: "action-item action-item__menutoggle", attrs: { "type": "tertiary-no-background", "aria-label": _vm.avatarAriaLabel }, on: { "click": _vm.toggleMenu }, scopedSlots: _vm._u([{ key: "icon", fn: function() {
    return [_vm.contactsMenuLoading ? _c("NcLoadingIcon") : _c("DotsHorizontal", { attrs: { "size": 20 } })];
  }, proxy: true }], null, false, 2617833509) }) : _vm.hasMenu ? _c("NcActions", { attrs: { "force-menu": "", "manual-open": "", "type": "tertiary-no-background", "container": _vm.menuContainer, "open": _vm.contactsMenuOpenState, "aria-label": _vm.avatarAriaLabel }, on: { "update:open": function($event) {
