require('../assets/NcBreadcrumbs-CFRjXqRg.css');
"use strict";
...
const NcBreadcrumb = require("./NcBreadcrumb-DrlhnNbV.cjs");
  ...
  for (let index = slots.length - 1; index >= 0; index--) {
    ...
    if (isHtmlElement || !isVueComponent || isForbiddenComponent) {
      if (isHtmlElement || isForbiddenComponent) {
        Vue__default.default.util.warn(`${isHtmlElement ? node.tag : node.componentOptions.tag} is not allowed inside the ${vm.$options.name} component`, vm);
      }
...
const _sfc_main = {
  ...
  components: {
    ...
    NcActionRouter: Components_NcActionRouter,
    NcActionLink: Components_NcActionLink,
    NcBreadcrumb: NcBreadcrumb.NcBreadcrumb,
    IconFolder
  },
  props: {
    /**
  ...
  created() {
    window.addEventListener("resize", debounce__default.default(() => {
      this.handleWindowResize();
    }, 100));
    eventBus.subscribe("navigation-toggled", this.delayedResize);
  },
  mounted() {
    this.handleWindowResize();
  },
  ...
  beforeDestroy() {
    ...
    eventBus.unsubscribe("navigation-toggled", this.delayedResize);
  },
  methods: {
    /**
  ...
  render(h) {
    ...
    this.$slots.default.forEach((vnode) => {
      ...
      if (vnode?.type === vueFrag.Fragment) {
    ...
    Vue__default.default.set(breadcrumbs[0].componentOptions.propsData, "icon", this.rootIcon);
    Vue__default.default.set(breadcrumbs[0].componentOptions.propsData, "ref", "breadcrumbs");
    const breadcrumbsRefs = {};
    breadcrumbs.forEach((crumb, index) => {
      Vue__default.default.set(crumb, "ref", `crumb-${index}`);
...
exports.NcBreadcrumbs = NcBreadcrumbs;
